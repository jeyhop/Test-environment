...
const updateHud = () => {
  ...
  activeCount.textContent = `${state.selectedIds.size} / ${getTaxNodes().length}`;
...
const createLinkPath = (from, to) => {
  ...
  const isRightCluster = from.x > 760 || to.x > 760;
  const curveX = Math.max(70, Math.abs(dx) * (isRightCluster ? 0.22 : 0.28));
  const curveY = Math.max(80, Math.abs(dy) * (isRightCluster ? 0.22 : 0.34));
...
const drawTree = () => {
  ...
  state.dataset.groups.forEach((group) => {
    group.taxes.forEach((tax) => {
      ...
      const clusterClass = tax.position.x > 760 || group.position.x > 760 ? 'right-link' : 'left-link';
      const path = createSvgElement('path', {
        class: `link-line ${clusterClass} ${isHighlighted ? 'active' : ''} ${isSelected ? 'selected' : ''}`,
  ...
  const makeNode = (item, radius) => {
    const scopeClass =
      item.kind === 'tax'
        ? `${item.scope.toLowerCase()}-tax`
        : `${item.label.toLowerCase().split(' ')[0]}-group`;

    const node = createSvgElement('g', {
      class: `node ${item.kind} ${scopeClass} ${state.activeId === item.id ? 'active' : ''} ${state.selectedIds.has(item.id) ? 'selected' : ''}`,
  ...
  state.dataset.groups.forEach((group) => makeNode({ ...group, kind: 'group' }, 60));
  state.dataset.groups.forEach((group) => group.taxes.forEach((tax) => makeNode({ ...tax, kind: 'tax' }, 42)));
};
...
const setView = (nextView) => {
  state.view = {
    scale: clamp(nextView.scale, 0.72, 2.25),
    x: clamp(nextView.x, -460, 460),
    y: clamp(nextView.y, -280, 280)
  };
  applyPanZoom();
};
